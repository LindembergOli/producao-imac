...
import type { ProductionSpeedRecord, DailyProduction, Product } from '../types';
...
const ProductionRecordForm: React.FC<{
    ...
    const [formData, setFormData] = useState<Omit<ProductionSpeedRecord, 'id' | 'totalProgramado' | 'totalRealizado' | 'velocidade'>>({
        mesAno: record?.mesAno || mesAnoOptions[0],
    ...
    useEffect(() => {
        setAvailableProducts(products.filter(p => p.sector === formData.sector));
    ...
    const handleDailyChange = (dayIndex: number, field: keyof DailyProduction, value: string) => {
        ...
        newDailyProduction[dayIndex] = { ...newDailyProduction[dayIndex], [field]: Number(sanitizedValue) };
        ...
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <select value={formData.mesAno} onChange={e => setFormData({ ...formData, mesAno: e.target.value })} className={inputClass}>
            ...
            <div>
                ...
                <input type="number" value={formData.metaMes || ''} onFocus={e => e.target.select()} onChange={e => setFormData({ ...formData, metaMes: Number(sanitizeFormInput(e.target.value)) })} className={inputClass} />
            </div>
            ...
            <div className="border-t dark:border-slate-700 pt-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 p-4 bg-gray-50 dark:bg-slate-900/50 rounded-b-md">
                    {Array.from({ length: dayTabs[activeDayTab].range[1] - dayTabs[activeDayTab].range[0] + 1 }, (_, i) => i + dayTabs[activeDayTab].range[0]).map(dayIndex => (
                        <div key={dayIndex}>
...
const ProductionSpeed: React.FC<ProductionSpeedProps> = ({ products, records, setRecords, isDarkMode }) => {
    ...
    const filteredTableRecords = useMemo(() => {
        return records.filter(rec => {
            if (tableFilters.mesAno && rec.mesAno !== tableFilters.mesAno) return false;
    ...
    const handleSave = (data: Omit<ProductionSpeedRecord, 'id'>) => {
        if (currentRecord) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input
                            ...
                            onChange={(e) => {
                                if (!e.target.value) {
                                    setTableFilters({ ...tableFilters, mesAno: '' });
                                } else {
                                    const [y, m] = e.target.value.split('-');
                                    setTableFilters({ ...tableFilters, mesAno: `${m}/${y}` });
                                }
                            }}
