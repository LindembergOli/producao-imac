...
// Helper to count business days (Mon-Fri) with safety
const countBusinessDays = (startDate: Date, endDate: Date) => {
    if (!startDate || !endDate) return 0;
    if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) return 0;
    
    // Clone dates to strip time and avoid mutation
...
const Dashboard: React.FC<DashboardProps> = ({ speedRecords, lossRecords, errorRecords, maintenanceRecords, absenteeismRecords, employees, isDarkMode }) => {
  ...
  // Initial state: Empty strings to show "All Time"
  const [dateRange, setDateRange] = useState({
    start: '',
    end: ''
  });

  // Filter records for dashboard view
  ...
  const kpiData = useMemo(() => {
    ...
    // Absenteísmo - Dynamic Calculation for Rate
    const totalDaysAbsent = filteredAbsenteeism.reduce((sum, r) => sum + r.daysAbsent, 0);
    
    // Determine Effective Date Range for Business Day Count logic
    let effectiveStartDate: Date;
    let effectiveEndDate: Date;

    if (dateRange.start) {
        effectiveStartDate = new Date(dateRange.start);
    } else if (filteredAbsenteeism.length > 0) {
        const minTime = Math.min(...filteredAbsenteeism.map(r => new Date(r.date).getTime()));
        effectiveStartDate = new Date(minTime);
    } else {
    ...
    if (dateRange.end) {
        ...
        const maxTime = Math.max(...filteredAbsenteeism.map(r => new Date(r.date).getTime()));
        effectiveEndDate = new Date(maxTime);
    } else {
  ...
  const chartData = useMemo(() => {
    ...
    const employeesPerSector = employees.reduce((acc, emp) => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 transition-colors duration-300">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
              <div>
                  <label htmlFor="start-date" className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider ml-1">Data Início</label>
                  <input 
                    type="date" 
                    id="start-date" 
                    value={dateRange.start}
                    onChange={(e) => setDateRange({...dateRange, start: sanitizeFormInput(e.target.value)})}
                    className="mt-1 block w-full rounded-xl border-slate-200 dark:border-slate-600 shadow-sm focus:border-imac-primary focus:ring-2 focus:ring-imac-primary/20 text-sm p-2.5 bg-slate-50/50 dark:bg-slate-700/50 text-slate-800 dark:text-slate-100 transition-all"
                  />
              </div>
              <div>
                  <label htmlFor="end-date" className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider ml-1">Data Fim</label>
                  <input 
                    type="date" 
                    id="end-date" 
                    value={dateRange.end}
                    onChange={(e) => setDateRange({...dateRange, end: sanitizeFormInput(e.target.value)})}
                    className="mt-1 block w-full rounded-xl border-slate-200 dark:border-slate-600 shadow-sm focus:border-imac-primary focus:ring-2 focus:ring-imac-primary/20 text-sm p-2.5 bg-slate-50/50 dark:bg-slate-700/50 text-slate-800 dark:text-slate-100 transition-all"
                  />
              </div>
          </div>
        ...
        <ChartContainer title="Perdas de Produção por Setor">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: -10, bottom: 20 }}>
              ...
              <Tooltip 
                contentStyle={tooltipStyle}
                formatter={(value, name) => name === 'Custo (R$) Perdas' ? [`R$ ${Number(value).toFixed(2)}`, 'Custo'] : [value, 'Perdas (KG)']} 
              />
              <Legend wrapperStyle={{fontSize: "12px", paddingTop: "10px"}} iconType="circle"/>
        ...
        <ChartContainer title="Erros de Produção por Setor">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: -10, bottom: 20 }}>
                ...
                <Tooltip 
                    contentStyle={tooltipStyle}
                    formatter={(value, name) => name === 'Custo (R$) Erros' ? [`R$ ${Number(value).toFixed(2)}`, 'Custo'] : [value, 'Qtd']} 
                />
                <Legend wrapperStyle={{fontSize: "12px", paddingTop: "10px"}} iconType="circle"/>
