...
const formatDateSafe = (dateString: string) => {
  try {
    ...
    // Criar data sem ajuste de timezone para exibição correta
...
const AbsenteeismRecordForm: React.FC<{
  ...
  useEffect(() => {
    if (formData.sector) {
      setAvailableEmployees(employees.filter(e => e.sector === formData.sector));
  ...
  return (
    <div className="space-y-6 text-imac-text dark:text-slate-200">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium mb-1">Dia da Falta *</label>
        ...
        <div>
          ...
          <input type="number" min="1" value={formData.daysAbsent || ''} onFocus={e => e.target.select()} onChange={e => setFormData({ ...formData, daysAbsent: Number(sanitizeFormInput(e.target.value)) })} className={inputClass} />
        </div>
        <div>
...
const Absenteeism: React.FC<AbsenteeismProps> = ({ employees, records, setRecords, isDarkMode }) => {
  ...
  const filteredOverviewRecords = useMemo(() => {
    ...
    return records.filter(rec => {
      ...
      if (overviewFilters.start) {
        const startParts = overviewFilters.start.split('-').map(Number);
        if (startParts.length >= 3) {
          const [sy, sm, sd] = startParts;
          if (sy !== undefined && sm !== undefined && sd !== undefined) {
      ...
      if (overviewFilters.end) {
        const endParts = overviewFilters.end.split('-').map(Number);
        if (endParts.length >= 3) {
          const [ey, em, ed] = endParts;
          if (ey !== undefined && em !== undefined && ed !== undefined) {
  ...
  const filteredTableRecords = useMemo(() => {
    ...
    return records.filter(rec => {
      ...
      if (tableFilters.mesAno) {
        ...
        const parts = rec.date.split('-').map(Number);
        if (parts.length >= 2) {
          const [recY, recM] = parts;
          if (recM !== Number(m) || recY !== Number(y)) return false;
        } else {
          return false;
        }
      }
  ...
  const kpiData = useMemo(() => {
    ...
    if (overviewFilters.start) {
      const parts = overviewFilters.start.split('-').map(Number);
      if (parts.length >= 3) {
        const [y, m, d] = parts;
    ...
    if (overviewFilters.end) {
      const parts = overviewFilters.end.split('-').map(Number);
      if (parts.length >= 3) {
        const [y, m, d] = parts;
        if (y !== undefined && m !== undefined && d !== undefined) {
  ...
  const handleSave = (data: Omit<AbsenteeismRecord, 'id'>) => {
    if (currentRecord) {
  ...
  const renderOverview = () => (
    <div className="space-y-6">
      <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm transition-colors">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label htmlFor="start-date" className="text-sm font-medium text-gray-500 dark:text-gray-400">Data Início</label>
            <input
              type="date"
              id="start-date"
              value={overviewFilters.start}
              onChange={(e) => setOverviewFilters({ ...overviewFilters, start: e.target.value })}
              className="mt-1 block w-full rounded-md border-gray-200 dark:border-slate-600 shadow-sm focus:border-imac-primary focus:ring-imac-primary sm:text-sm p-2 bg-white dark:bg-slate-700 dark:text-white"
            />
          </div>
          <div>
            <label htmlFor="end-date" className="text-sm font-medium text-gray-500 dark:text-gray-400">Data Fim</label>
            <input
              type="date"
              id="end-date"
